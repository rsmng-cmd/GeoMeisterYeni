...
export class MapEngine {
  ...
  init(containerId, modeConfig) {
    ...
    const mapZoom = this.modeConfig?.mapZoom || { start: 2, min: 2, max: 12 };
    const maxZoom = mapZoom.max || 10;
    const minZoom = Math.max(1, (mapZoom.min || 2) - (isPortrait ? 1 : 0));
  ...
  flyToDefault() {
    ...
    // Yavaş animasyon yerine anında (0ms) harita görünümünü sıfırla
